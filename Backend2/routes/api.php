...
// SUPPORT BOTH /v1 AND /api/v1 TO PREVENT 404 ON SERVER
$paths = ['v1', 'api/v1'];

foreach ($paths as $p) {
    Route::prefix($p)->group(function () use ($p) {
        
        // Add a suffix to route names to prevent "Another route has already been assigned name" error
        $nameSuffix = ($p === 'v1') ? '.v1' : '.api';
        ...
        // ADMIN ROUTES (We name these manually to avoid conflicts)
        Route::prefix('admin')->middleware(['auth:sanctum', 'admin'])->group(function () {
            Route::get('/dashboard', [\App\Http\Controllers\API\V1\Admin\DashboardController::class, 'index']);
            
            // Use different names for resources to avoid 'products.index' collision
